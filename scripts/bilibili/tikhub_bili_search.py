...
DEFAULT_PATH = "/api/v1/bilibili/app/fetch_search_all"
...
def parse_args():
    examples = (
        ...
        "  python tikhub_bili_search.py --keyword genshin --order 0\n"
    )
    parser = argparse.ArgumentParser(
        description="TikHub Bilibili search all client",
    ...
    parser.add_argument("--page-size", type=int, help="Results per page")
    parser.add_argument("--order", type=int, help="Sort order (0=comprehensive)")
...
def apply_env_defaults(args):
    ...
    if keyword is None:
        keyword = _env_str("TIKHUB_BILI_KEYWORD", "")
    args.keyword = keyword

    if args.page is None:
        args.page = _env_int("TIKHUB_BILI_PAGE", 1)
    if args.page_size is None:
        args.page_size = _env_int("TIKHUB_BILI_PAGE_SIZE", 20)
    if args.order is None:
        args.order = _env_int("TIKHUB_BILI_ORDER", 0)
    if args.host is None:
        args.host = _env_str("TIKHUB_HOST", DEFAULT_HOST)
    if args.path is None:
        args.path = _env_str("TIKHUB_BILI_PATH", DEFAULT_PATH)
    if args.timeout is None:
        args.timeout = _env_int("TIKHUB_TIMEOUT", 30)
    return args


def fetch_search_all(token, params, host=DEFAULT_HOST, path=DEFAULT_PATH, timeout=30):
